...
{
    /// <summary>
    /// The main CSP state object, containing all functions and persistent states.
    /// </summary>
    ...
    {
        /// <summary>
        /// A simple list of all variables in the CSP instance
        /// </summary>
        readonly List<R> variables = new List<R>();
        /// <summary>
        /// The initial domains of each variable
        /// </summary>
        readonly Dictionary<R, IEnumerable<T>> domains = new Dictionary<R, IEnumerable<T>>();
        /// <summary>
        /// The constraints on each variable, where constraints[i][j] is a list of possible combinations of the values of i and j.
        /// </summary>
        readonly Dictionary<R, Dictionary<R, IEnumerable<(T, T)>>> constraints = new Dictionary<R, Dictionary<R, IEnumerable<(T, T)>>>();
        /// <summary>
        ...
        public void AddVariable(R name, IEnumerable<T> domain)
        {
            variables.Add(name);
            domains[name] = domain;
            constraints[name] = new Dictionary<R, IEnumerable<(T, T)>>();
        }
        /// <summary>
        ...
        public void PrintConstraints()
        {
            foreach (var kvp in constraints)
            {
                ...
                {
                    Console.Write($"Arc: {kvp.Key} -> {kvp2.Key}: ");
                    foreach (var pair in kvp2.Value)
                    {
        ...
        /// <param name="filter">Constraint function</param>
        public void AddOneWayConstraint(R i, R j, Func<T, T, bool> filter)
        {
            if (!constraints[i].ContainsKey(j))
            {
                constraints[i][j] = Utils.GetAllPairs(domains[i], domains[j]);
            }
            constraints[i][j] = constraints[i][j].Where(item => filter(item.Item1, item.Item2));
        }
        /// <summary>
        ...
        public bool Revise(Dictionary<R, IEnumerable<T>> assignment, R i, R j)
        {
            ...
            {
                if (!constraints[i][j].Any(constraint => constraint.Item1.Equals(x) && assignment[j].Contains(constraint.Item2)))
                {
                    revised = true;
                    illegal.Add(x);
                }
